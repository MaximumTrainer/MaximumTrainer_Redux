...
| `login_screen_tests.pro` | `testOfflineLogin`, `testIntervalsIcuOAuthUrlGeneration`, `testIntervalsIcuApiLogin`, `testDialogLoginInitialState`, `testDialogLoginOfflineFlow`, `testDialogLoginIntervalsIcuButton`, `testDialogLoginIntervalsIcuOAuthDialog`, `testIntervalsIcuOAuthFlowLoopback` | Login dialog state machine; offline login path; OAuth2 URL generation; Intervals.icu API login; system-browser loopback OAuth flow |
